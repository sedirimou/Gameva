...
export default function RegisterComponent({ onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        handleApiSuccess('Registration successful! Welcome to Gamava.');
        onNavigate('login');
      } else {
        const error = await response.json();
        handleApiError(new Error(error.error || 'Registration failed. Please try again.'));
      }
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <h1 className="text-xl md:text-3xl font-bold text-white mb-1 md:mb-2">Sign Up</h1>
      <p className="text-white/80 mb-4 md:mb-6 text-sm md:text-base">Create your account to get started</p>
